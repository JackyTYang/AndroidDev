...
public class MainActivity extends AppCompatActivity {
    ...
    private void showMessages(List<Message> messageList) {
        Log.d(TAG, "repo list add " + messageList.size());
...
//     * @param student_id
     */
    private void requestBase(String studentId) {
        new Thread(new Runnable() {
            @Override
            public void run() {
                MessageListResponse messageListResponse = baseGetMessagesFromRemote(studentId);
                if(messageListResponse!=null){
                    new Handler(getMainLooper()).post(new Runnable() {
                        @Override
                        public void run() {
                            Log.d("test","10000");
                            List<Message> result = messageListResponse.feeds;
                            showMessages(result);
                        }
                    });
                }
